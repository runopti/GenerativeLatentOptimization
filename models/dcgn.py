...
class DCGN(object):
    def __init__(self, config):
        ...
        self.batch_z = tf.nn.embedding_lookup(self.input, self.batch_indices) # [batch_size, n_input]
        # self.batch_id = 0
    ...
    def _create_layers(self):
        self.conv_layers.append(L.Conv2D_transpose(in_channels=self.n_input*4, output_shape=[self.batch_size, 7, 7, self.n_input*2]))
        self.bn_layers.append(L.BN())
        self.act_layers.append(L.ReLU())

        self.conv_layers.append(L.Conv2D_transpose(in_channels=self.n_input*2, output_shape=[self.batch_size, 14, 14, self.n_input*1]))
        self.bn_layers.append(L.BN())
        self.act_layers.append(L.ReLU())

        self.conv_layers.append(L.Conv2D_transpose(in_channels=self.n_input*1, output_shape=[self.batch_size, 28, 28, 1]))
        self.bn_layers.append(L.BN())
        self.act_layers.append(L.ReLU())

    def _inference(self, input):
        # reshape input
        l = L.Linear(n_in=self.n_input, n_out=self.n_input*4*4*4, name_scope="linear")
        z = l.add(input)
        h = tf.reshape(z, [-1, 4, 4, self.n_input*4])
        # convolution tranpose
        for c, bn, act in zip(self.conv_layers,self.bn_layers,self.act_layers):
            h = c.add(h)
            h = bn.add(h)
            h = act.add(h)
        return tf.reshape(h, [-1, self.n_output])
